...
use iptools::{iprange, ipv4, ipv6};
use js_sys::BigInt;
use std::{format, string::String, vec, vec::Vec};
use wasm_bindgen::prelude::*;

fn js_err(err: impl core::fmt::Debug) -> JsValue {
...
pub fn ipv4_ip2long(ip: &str) -> Result<u32, JsValue> {
    ipv4::ip2long(ip).map_err(js_err)
}
...
pub fn ipv4_ip2hex(ip: &str) -> Result<String, JsValue> {
    ipv4::ip2hex(ip).map_err(js_err)
}

#[wasm_bindgen]
pub fn ipv4_hex2ip(hex: &str) -> Result<String, JsValue> {
    ipv4::hex2ip(hex).map_err(js_err)
}
...
pub fn ipv6_ip2long(ip: &str) -> Result<BigInt, JsValue> {
    ipv6::ip2long(ip).map(BigInt::from).map_err(js_err)
}
